...
export default function Home() {
  const [feed, setFeed] = useState<PerksFeed | null>(null);
  ...
  useEffect(() => {
    fetch(`${BASE}/perks.json`)
      .then((r) => r.json())
      .then(setFeed)
      .catch(() => setFeed({ generated_at: "", count: 0, by_category: {} as any, perks: [] }));
  }, []);
  ...
  const updated = feed?.generated_at
    ? new Date(feed.generated_at).toLocaleDateString(undefined, {
        year: "numeric",
        month: "short",
        day: "numeric",
      })
